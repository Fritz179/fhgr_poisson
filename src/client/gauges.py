...
def draw_thermometer(top_left, width, height, img, temp_c):
    """Draw a simple vertical thermometer and digital readout. Returns bounding box (x, y, w, h)."""
    x, y = top_left
    w = max(20, int(width))
    h = max(80, int(height))

    tube_width = max(6, int(w * 0.35))
    bulb_radius = max(tube_width, w // 2)
    tube_x1 = x + (w - tube_width) // 2
    tube_x2 = tube_x1 + tube_width
    tube_y1 = y
    tube_y2 = y + h - bulb_radius

    cv2.rectangle(img, (tube_x1, tube_y1), (tube_x2, tube_y2), (200, 200, 200), 2, cv2.LINE_AA)
    ...
    fill_color = (0, 80, 255)
    cv2.rectangle(img, (tube_x1 + 2, fill_y1), (tube_x2 - 2, tube_y2 - 2), fill_color, -1, cv2.LINE_AA)

    bulb_center = (x + w // 2, y + h - bulb_radius)
    cv2.circle(img, bulb_center, bulb_radius, (200, 200, 200), 2, cv2.LINE_AA)
    cv2.circle(img, bulb_center, bulb_radius - 3, fill_color, -1, cv2.LINE_AA)

    # Major/minor ticks along the left side
    major_tick_len = max(6, int(w * 0.32))
    minor_tick_len = max(3, int(major_tick_len * 0.6))
    tick_x2 = tube_x1 - 4
    tick_x1_major = tick_x2 - major_tick_len
    tick_x1_minor = tick_x2 - minor_tick_len
    tick_thickness = max(1, w // 12)
    span = (TEMP_MAX - TEMP_MIN)
    label_font_size = max(8, int(w * 0.25))
    digital_font_size = max(10, int(w * 0.35))
    label_font = load_font(label_font_size)
    digital_font = load_font(digital_font_size)

    for t in range(int(TEMP_MIN), int(TEMP_MAX) + 1, 10):
        ratio_t = (t - TEMP_MIN) / span
        y_tick = tube_y2 - int((tube_y2 - tube_y1) * ratio_t)
        cv2.line(img, (tick_x1_major, y_tick), (tick_x2, y_tick), (180, 180, 180), tick_thickness, cv2.LINE_AA)
        # Label big ticks
        label = f"{t:+}"
        # Defer actual text drawing to PIL overlay below; store positions
        # mid-height baseline alignment handled after overlay creation
        mid_t = t + 5
        if mid_t < TEMP_MAX:
            ratio_m = (mid_t - TEMP_MIN) / span
            y_mid = tube_y2 - int((tube_y2 - tube_y1) * ratio_m)
            cv2.line(img, (tick_x1_minor, y_mid), (tick_x2, y_mid), (150, 150, 150), max(1, tick_thickness - 1), cv2.LINE_AA)

    # Digital readout below the thermometer
    # Draw labels and digital readout with PIL for crisper text
    pil_img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(pil_img)
    ...
    txt_h = bbox[3] - bbox[1]
    text_x = x + (w - txt_w) // 2
    text_y = y + h + txt_h + max(4, w // 6) - txt_h
